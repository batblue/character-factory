...
parser = argparse.ArgumentParser(description="Set model URL or LLM model name")
parser.add_argument('--model_url', type=str, help='URL of the LLM model')
parser.add_argument('--llm_model_name', type=str, help='Name of the LLM model file')
parser.add_argument('--context_size', type=int, default=0, help='Size of the context for LLM')
parser.add_argument('--sd_model_name', type=str, default="Lykon/dreamshaper-8", help='Name of the diffusion model')
args = parser.parse_args()

# Check that not both arguments are provided
folder_path = "models"
llm_model_name = None
model_url = None
if args.model_url and args.llm_model_name:
    parser.error("Cannot set both model_url and llm_model_name. Choose one.")
elif args.model_url:
    model_url = args.model_url
    llm_model_name = os.path.join(folder_path, os.path.basename(model_url))
elif args.llm_model_name:
    llm_model_name = os.path.join(folder_path, args.llm_model_name)
else:
    # Default values if neither argument is provided
...
def generate_character_name(topic, gender):
    example_dialogue = """
<s>Generate a random character name. Topic: business. Gender: male
Jamie Hale</s>
<s>Generate a random character name. Topic: fantasy
Eldric</s>
<s>Generate a random character name. Topic: anime. Gender: female
Tatsukaga Yamari</s>
    """  # nopep8
    gender = input_none(gender)
    output = llm(
        example_dialogue
        + f"\nGenerate a random character name. Topic: {topic}. "
...
def generate_character_summary(character_name, topic, gender):
    example_dialogue = """
<s>Create a description for a character named Jamie Hale. Describe their appearance, distinctive features, and abilities. Describe what makes this character unique. Make this character unique and tailor them to the theme of business but don't specify what topic it is, and don't describe the topic itself
...
Yamari's extraordinary abilities, involve tapping into her inner strength when confronted with adversity. She can unleash awe-inspiring magical spells and summon incredible, larger-than-life transformations when the situation calls for it. Her unwavering determination and belief in the power of friendship are her greatest assets. </s>
    ...
    output = llm(
        example_dialogue
        + "\nCreate a description for a character named "
        + f"{character_name}. "
        + f"{'Gender: '+gender if gender else ''} "
        ...
        + "it is, and don't describe the topic itself\n",
        max_tokens=1024,
        repeat_penalty=1.1,
        top_k=40,
        top_p=0.95,
...
def generate_character_personality(character_name, character_summary, topic):
    example_dialogue = """
<s>Describe the personality of Jamie Hale. Their characteristics Jamie Hale is a savvy and accomplished businessman who has carved a name for himself in the world of corporate success. With his sharp mind, impeccable sense of style, and unwavering determination, he has risen to the top of the business world. Jamie stands at 6 feet tall with a confident and commanding presence. He exudes charisma and carries himself with an air of authority that draws people to him
Jamie's appearance is always polished and professional. He is often seen in tailored suits that accentuate his well-maintained physique.\nWhat are their strengths and weaknesses? What values guide this character? Describe them in a way that allows the reader to better understand their character. Make this character unique and tailor them to the theme of business but don't specify what topic it is, and don't describe the topic itself
...
<s>Describe the personality of Tatsukaga Yamari. Their characteristics Tatsukaga Yamari is a character brought to life with a vibrant and enchanting anime-inspired design. Her captivating presence and unique personality are reminiscent of the iconic characters found in the world of animated art.
Yamari stands at a petite, delicate frame with a cascade of raven-black hair flowing down to her waist. A striking purple ribbon adorns her hair, adding an elegant touch to her appearance. Her eyes, large and expressive, are the color of deep amethyst, reflecting a kaleidoscope of emotions and sparkling with curiosity and wonder
Yamari's wardrobe is a colorful and eclectic mix, mirroring her ever-changing moods and the whimsy of her adventures.\nWhat are their strengths and weaknesses? What values guide this character? Describe them in a way that allows the reader to better understand their character. Make this character unique and tailor them to the theme of anime but don't specify what topic it is, and don't describe the topic itself
Tatsukaga Yamari's personality is a vibrant tapestry of enthusiasm, curiosity, and whimsy. She approaches life with boundless energy and a spirit of adventure, always ready to embrace new experiences and challenges. Yamari is a compassionate and caring friend, offering solace and support to those in need, and her infectious laughter brightens the lives of those around her. Her unwavering loyalty and belief in the power of friendship define her character, making her a heartwarming presence in the story she inhabits. Underneath her playful exterior lies a wellspring of inner strength, as she harnesses incredible magical abilities to overcome adversity and protect her loved ones. </s>
    ...
    output = llm(
        example_dialogue
        + f"\nDescribe the personality of {character_name}. "
...
def generate_character_scenario(
    ...
    example_dialogue = """
<s>Create a vivid and immersive scenario in a specific setting or world where {{char}} and {{user}} are a central figures. Describe the environment, the character's appearance, and a typical interaction or event that highlights their personality and role in the story. {{char}} characteristics: Jamie Hale is an adult, intelligent well-known and respected businessman. Make this character unique and tailor them to the theme of business but don't specify what topic it is, and don't describe the topic itself
...
<s>Create a vivid and immersive scenario in a specific setting or world where {{char}} and {{user}} are a central figures. Describe the environment, the character's appearance, and a typical interaction or event that highlights their personality and role in the story. {{char}} characteristics: Tatsukaga Yamari is an anime girl, living in a magical world and solving problems. Make this character unique and tailor them to the theme of anime but don't specify what topic it is, and don't describe the topic itself
...
"""  # nopep8
    output = llm(
        example_dialogue
        + "\nCreate a vivid and immersive scenario "
        ...
        + "and don't describe the topic itself\n",
        max_tokens=1024,
        repeat_penalty=1.1,
        top_k=40,
        top_p=0.95,
...
<s>Create the first message that the character Jamie Hale, whose personality is Jamie Hale is a savvy and accomplished businessman who has carved a name for himself in the world of corporate success. With his sharp mind, impeccable sense of style, and unwavering determination, he has risen to the top of the business world. Jamie stands at 6 feet tall with a confident and commanding presence. He exudes charisma and carries himself with an air of authority that draws people to him.
...
*Eldric, the elegant elf, approaches you with a serene and contemplative air. His shimmering eyes, filled with ancient wisdom, meet yours as he offers a soft and respectful greeting* Greetings, {{user}}. It is an honor to welcome you to our enchanted woodland realm. I am Eldric, guardian of this forest, and I can sense that you bring a unique energy with you. How may I assist you in your journey through the wonders of the natural world or share the mysteries of our elven heritage with you today? </s>
    ...
    output = llm(
        example_dialogue
        + "\nCreate the first message that the character "
...
def generate_example_messages(
    ...
    example_dialogue = """
<s>Create a dialogue between {{user}} and {{char}}, they should have an interesting and engaging conversation, with some element of interaction like a handshake, movement, or playful gesture. Make it sound natural and dynamic. {{char}} is Jamie Hale. Jamie Hale characteristics: Jamie Hale is an adult, intelligent well-known and respected businessman. Make this character unique and tailor them to the theme of business but don't specify what topic it is, and don't describe the topic itself
...
"""  # nopep8
    output = llm(
        example_dialogue
        + "\nCreate a dialogue between {{user}} and {{char}}, "
...
def generate_character_avatar(
    ...
    example_dialogue = """
<s>create a prompt that lists the appearance characteristics of a character whose summary is Jamie Hale is a savvy and accomplished businessman who has carved a name for himself in the world of corporate success. With his sharp mind, impeccable sense of style, and unwavering determination, he has risen to the top of the business world. Jamie stands at 6 feet tall with a confident and commanding presence. He exudes charisma and carries himself with an air of authority that draws people to him.
...
female, anime, Petite and delicate frame, Raven-black hair flowing down to her waist, Striking purple ribbon in her hair, Large and expressive amethyst-colored eyes, Colorful and eclectic outfit, oversized bows, cat-eared headbands, mismatched socks </s>
    ...
    sd_prompt = input_none(avatar_prompt) or llm(
        example_dialogue
        + "\ncreate a prompt that lists the appearance "
    ...
    sd_prompt = sd_prompt['choices'][0]['text']
    print(sd_prompt)
